...
\newcommand{\exercisetopic}{Exercise Collection}
% *****************************************************************************
...
<<echo = FALSE, eval = FALSE>>=
lung <- read.csv("data/perulung_ems.csv", sep = ";")
head(lung)
tail(lung)
str(lung)
@
...
# OPTION II:
# change the whole vector at once (only one line of code)
levels(bac$y)
summary(bac$y)
levels(bac$y) <- c("n", "y", "n", "y")
summary(bac$y)
levels(bac$y) <- c("no", "yes")
levels(bac$y)
summary(bac)
...
summary(bac$child_weight)
bac$child_weight[bac$child_weight == 302.8] <- 30.28
summary(bac)
class(bac$child_weight)
hist(bac$child_weight)
boxplot(bac$child_weight)

# rows below 2.5 kg
which(bac$child_weight < 2.5)
# all observations below 2.5 kg
bac[which(bac$child_weight < 2.5), ]
# delete the observations
dim(bac)
bac <- bac[-which(bac$child_weight < 2.5), ] # dim(bac)
dim(bac)
summary(bac)
@
\item Do all variables have the correct data type (numeric, integer, factor)?
...
x1[which(is.na(x1))] <- 0
x1
# # OPTION III: with the ifelse statement
x1 <- c(rnorm(10,5,2), NA, 5:12, NA, 6, 7.5, NA)
...
% \ex{Data import to R: \texttt{water\_errors.csv}}
% \vspace*{-1cm}
% \begin{enumerate}[(a)]
...
\vspace*{-1cm}
\begin{enumerate}[(a)]
\item Load the data set \texttt{ToothGrowth} within R and apply the two-sided two
sample t-test to suitable variables of the data set.
<<eval = FALSE>>=
data(ToothGrowth)
@
<<echo = FALSE, eval = FALSE>>=
...
summary(lm.mod1)
@
\item Read in the data set \texttt{perulung\_ems} and apply the two-sided t-test
to suitable variables of the \texttt{perulung\_ems} data set and interpret the results.
<<echo = FALSE, eval = FALSE>>=
...
\texttt{bacteria} containing only the observations taken in week 2 (cf. Exercise 3).
...
\texttt{trt.new} variable (cf. Exercise 5) with the two levels \texttt{treated}
and \texttt{drug}.
<<echo = FALSE, eval = FALSE>>=
bacteria$trt.new <- bacteria$trt
levels(bacteria$trt.new) <- c("placebo", "drug", "drug")
...
summary(my.logreg)
exp(0.8473) # --> EXCACT SAME ESTIMATE AS FROM fisher.test
coef(my.logreg)
exp(coef(my.logreg))
@
...
\ex{Inside the square of the plot}
\vspace*{-1cm}
\begin{enumerate}[(a)]
\item Type \texttt{demo(graphics)} in your console and press enter.
This command shows you a nice demonstration of possible R graphics.
...
\vspace*{-1cm}
\begin{enumerate}[(a)]
\item Open the .R file \texttt{ANOVA\_with\_chickwts.R} from your RCourse folder
and have another look on how we applied the anova to the \texttt{chickwts}
data set. Check line for line.
...
\texttt{dose.factor} with the three levels \texttt{low}, \texttt{med} and
...
table(ToothGrowth$dose.factor)
@
\item Visualize the variable \texttt{len} per \texttt{dose.factor} level in a boxplot.
<<echo = FALSE, eval = FALSE>>=
...
% data set.
\item Reuse the commands from the lecture slides to fit a simple as well as a
multiple linear regression model to the data set of \texttt{perulung\_ems}.
Use \texttt{fev1} as your response variable $y$.
...
\ex{Linear Regression Model III}
\vspace*{-1cm}
\begin{enumerate}[(a)]
...
\ex{Combining everthing we have learnt}
...
@
\item Repeat the crosstable (2-by-2 table), Chi-squared test, Fisher's exact test
and a logistic regression (\texttt{glm}) for the subsets \textbf{separately}.
<<echo = FALSE, eval = FALSE>>=
# FEMALES
lepto.fem <- subset(lepto, gender == "female")
