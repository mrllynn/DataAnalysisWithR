...
@
%
\item Create a vector with some missing values. Then, find the number of missing
values and their position.
<<echo = FALSE, eval = FALSE>>=
...
%
\item Replace the missing values in \texttt{x1} with a 0, and check that no NAs are present
...
coef(lm.mod)
@
\item Apply the two-sided t-test to the \texttt{perulung\_ems} data set.
<<echo = FALSE, eval = FALSE>>=
