...
\begin{frame}[fragile]
\frametitle{Exercise 1: Terminology}
\end{frame}
...
\begin{frame}[fragile]
\frametitle{Exercise 2: Getting to know R and \texttt{chickwts}}
\end{frame}
...
\begin{frame}[fragile]
\frametitle{Exercise 3: Summary Statistics with \texttt{chickwts}}
\end{frame}
...
\begin{frame}[fragile]
\frametitle{Exercise 4: Data import to R: perulung\_ems.csv}
...
\begin{frame}[fragile]
\frametitle{Exercise 5: Defining a new data frame}
\end{frame}
...
\begin{frame}[fragile]
\frametitle{Exercise 6: Get to know \texttt{bacteria} data set}
\end{frame}
...
% \usebackgroundtemplate{}
% \begin{frame}[fragile]
% \frametitle{Exercise 9A and 9B}
% \framesubtitle{Plausibility Checks \& Missing Values}
% \end{frame}
% 
% \begin{frame}[fragile]
...
% \vspace{0.2cm}
% By convention a $p$-value of less than 0.05 is considered to provide \textbf{reasonable
% evidence} for rejecting $H_0$. A $p$-value of between 0.05 and 0.1 might be
% considered as \textbf{weak evidence} against $H_0$. Values of less that 0.01 are generally
% considered as \textbf{very strong evidence} for rejecting $H_0$. It is {\bfseries always}
...
% \usebackgroundtemplate{\includegraphics[width=\paperwidth]{Printscreen_RLogo.png}}
% \begin{frame}[fragile]
% \frametitle{Side remark: Conditional Inference Trees (1/2)}
% \begin{itemize}
% \setlength\itemsep{1.2em}
% \item \texttt{ctree(...)} is a non-parametric class of regression trees embedding
% tree structured regression models into a well defined theory of conditional inference
% procedures.
% \item \textbf{Recursive partitioning algorithm:}
% \begin{itemize}
% \item partition the observations by univariate splits in a recursive way
% \item fit a constant model in each cell of the resulting partition
% \item $\rightarrow$ uses an information measure of node impurityto select the variables
...
% \usebackgroundtemplate{\includegraphics[width=\paperwidth]{Printscreen_RLogo.png}}
% \begin{frame}[fragile]
% \frametitle{Side remark: Conditional Inference Trees (2/2)}
% <<echo = TRUE, eval = TRUE, size = "scriptsize", out.width = "60%", fig.align="center", message = FALSE, warning = FALSE, cache = TRUE>>=
% library("partykit")
% library("party")
% data("ToothGrowth")
...
% \item Two-way Interactions in R
% \item Confounding
% \end{itemize}
% \end{frame}
% 
...
% \end{itemize}
% \begin{center}
% \includegraphics[width=0.6\linewidth]{F_test.png}
% \end{center}
% \end{frame}
% 
% \usebackgroundtemplate{\includegraphics[width=\paperwidth]{Printscreen_RLogo.png}}
...
% AIC = -2 \cdot \log(L) + 2p \nonumber
% \end{eqnarray}
% $L$ stands for the likelihood. $p$ stands for the number of parameters in the
% models (penalizes complex models).
% \end{frame}
% 
% \usebackgroundtemplate{}
% \begin{frame}[fragile]
...
% \usebackgroundtemplate{\includegraphics[width=\paperwidth]{Printscreen_RLogo.png}}
% \begin{frame}[fragile]
% \frametitle{\texttt{mod.hard.loc} (\texttt{mod3}) is the best!}
...
% <<echo = FALSE, eval = TRUE, size="scriptsize", out.width = "70%", fig.align="center">>=
% par(mfrow=c(1,1))
% plot(jitter(ToothGrowth$dose, factor = 0.2), ToothGrowth$len,
%      pch = 16, cex = 1.2, ylim = c(0, 35),
%      col = c("orange", "blue")[as.numeric(ToothGrowth$supp)])
% 
% mod1 <- lm(len ~ dose,  data = subset(ToothGrowth, ((dose != 2) & (supp == "OJ"))))
% mod2 <- lm(len ~ dose,  data = subset(ToothGrowth, ((dose != 2) & (supp == "VC"))))
% mod3 <- lm(len ~ dose,  data = subset(ToothGrowth, ((dose != 0.5) & (supp == "OJ"))))
% mod4 <- lm(len ~ dose,  data = subset(ToothGrowth, ((dose != 0.5) & (supp == "VC"))))
% 
% segments(x0 = 0.5, y0 = as.numeric(coef(mod1)[1] + 0.5*coef(mod1)[2]),
%          x1 = 1.0, y1 = as.numeric(coef(mod1)[1] + 1*coef(mod1)[2]),
%          lwd = 3, col = "orange", lty = 2)
% segments(x0 = 0.5, y0 = as.numeric(coef(mod2)[1] + 0.5*coef(mod2)[2]),
%          x1 = 1.0, y1 = as.numeric(coef(mod2)[1] + 1*coef(mod2)[2]),
%          lwd = 3, col = "blue", lty = 2)
% segments(x0 = 1.0, y0 = as.numeric(coef(mod3)[1] + 1*coef(mod3)[2]),
%          x1 = 2.0, y1 = as.numeric(coef(mod3)[1] + 2*coef(mod3)[2]),
%          lwd = 3, col = "orange", lty = 2)
% segments(x0 = 1.0, y0 = as.numeric(coef(mod4)[1] + 1*coef(mod4)[2]),
%          x1 = 2.0, y1 = as.numeric(coef(mod4)[1] + 2*coef(mod4)[2]),
%          lwd = 3, col = "blue", lty = 2)
% @
% \end{frame}
% 
...
% \begin{itemize}
% \setlength\itemsep{1.5em}
% \item Not adjusting / controlling for a confounding variable may lead to biased
...
% exposure (here: \texttt{sex}) and an outcome (here: \texttt{respsymptoms}). The
...
% \end{itemize}
% \begin{center}
% \includegraphics[width=0.6\paperwidth]{two_by_two_table.png}
% \end{center}
% \end{frame}
% 
% \usebackgroundtemplate{\includegraphics[width=\paperwidth]{Printscreen_RLogo.png}}
